...
const tabs = document.querySelectorAll('.tab[data-style]');
...
function renderCards(style) {
  ...
  moves.forEach((move, index) => {
    ...
    card.addEventListener('click', (e) => {
      ...
      if (cardsContainer.classList.contains('compact')) {
...
if (localStorage.getItem('compactMode') === '1') {
  ...
  compactBtn.textContent = '关闭极简';
}

compactBtn.addEventListener('click', () => {
  const isCompact = cardsContainer.classList.toggle('compact');
  compactBtn.textContent = isCompact ? '关闭极简' : '开启极简';
...
回复必须以<svg开头，以</svg>结尾，中间不要有任何其他文字。`;
    ...
    btn.disabled = true;
    try {
      const raw = stripMarkdownBlock(await callAI(prompt));
      if (field === 'desc') {
        document.getElementById('editDesc').value = raw.trim().replace(/^["']|["']$/g, '');
      } else if (field === 'tips') {
        const m = raw.match(/\[[\s\S]*\]/);
        if (!m) throw new Error('AI 返回格式不正确，请重试');
...
直接返回JSON对象（不要用markdown代码块包裹），格式如下：
{"desc":"30-50字中文描述","tips":["建议1","建议2","建议3","建议4"],"svg":"<svg>...</svg>"}

字段要求：
- desc: 一句话说明动作特征和视觉效果
- tips: 4条练习建议，每条10-15字
- svg: 120x140 viewBox动画SVG火柴人，#f5e642描边，stroke-width 2.5，用CSS animation或SMIL动画展示动作，循环播放1-2秒

注意：直接输出JSON，不要加\`\`\`json标记，不要加任何解释文字。`;
...
function parseAIResponse(text) {
  const cleaned = stripMarkdownBlock(text);
